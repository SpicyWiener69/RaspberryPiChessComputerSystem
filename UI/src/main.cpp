...
static char *selected_backend; /* Global simulator settings, defined in lv_linux_backend.c */
...
#endif
    if (server_init(server_msg_cb, NULL) != 0){
        die("error init server");
    }

    lv_timer_t* timer = lv_timer_create(poll_timer, 500,  NULL);
